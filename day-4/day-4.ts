...
readFile("day-4/input.txt", "utf8", (err, data) => {
  if (err) throw err;
  ...
  for (const line of lines) {
    ...
    let hasFullOverlap = checkFullOverlap(
      parseInt(start1, 10),
      parseInt(end1, 10),
      parseInt(start2, 10),
      parseInt(end2, 10)
    ...
    let hasPartialOverlap = checkPartialOverlap(
      parseInt(start1, 10),
      parseInt(end1, 10),
      parseInt(start2, 10),
      parseInt(end2, 10)
