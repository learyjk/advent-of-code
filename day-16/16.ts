...
const file = readFileSync("day-16/input.txt", "utf-8");

let rows = file.split("\n");

// map where key is valve name, value is flow rate
let valveFlowData: Map<string, number> = new Map();
// map where key is valve name, value is array of valve names (tunnels)
let valveGraph: Map<string, string[]> = new Map();
...
const getMaxFlow = (
  ...
  if (
    valveFlowData.get(currentValve) !== 0 &&
  ...
  for (let nextValve of valveGraph.get(currentValve)) {
    ...
    maxFlow = Math.max(
      maxFlow,
      getMaxFlow(nextValve, openedValves, timeRemaining - 1)
    );
  }
