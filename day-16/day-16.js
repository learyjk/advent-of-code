...
const file = (0, fs_1.readFileSync)("day-16/inputExample.txt", "utf-8");
let rows = file.split("\n");
...
let parseInput = (input) => {
    ...
    for (let line of input) {
        ...
        let flowRate = parseInt(line.substring(line.indexOf("=") + 1, line.indexOf(";")), 10);
...
const createEmpty2DMatrix = (numValves) => {
    const M = Array.from(Array(numValves), () => {
        return new Array(numValves).fill(Infinity);
    });
    return M;
};
const populatePossiblePaths = (pathMatrix, valveMap) => {
...
const getShortestPathsMatrix = (M) => {
...
const printPathMatrix = (pathMatrix, valveMap) => {
    let firstStr = "   ";
    let secondStr = "   ";
    valveMap.forEach((valveInfo, valve) => {
        firstStr += valve[0] + "  ";
...
let cache = {};
const findMaxFlow = (time, valve, valveMap, shortestPaths) => {
    let maxVal = 0;
    for (let i = 0; i < shortestPaths.length; i++) {
        for (let j = 0; j < shortestPaths[0].length; j++) {
            ...
            maxVal = Math.max(maxVal, findMaxFlow(remainingTime, valveStr, valveMap, shortestPaths) +
