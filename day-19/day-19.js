...
const file = (0, fs_1.readFileSync)("day-19/inputExample.txt", "utf-8");
let rows = file.split("\n");
console.log({ rows });
let blueprints = new Map();
// parse the input
for (let row of rows) {
    let [, blueprintNumber, oreRobotOreCost, clayRobotOreCost, obsidianRobotOreCost, obsidianRobotClayCost, geodeRobotOreCost, geodeRobotObsidianCost,] = row.match(/(\d+): Each ore robot costs (\d+) ore. Each clay robot costs (\d+) ore. Each obsidian robot costs (\d+) ore and (\d+) clay. Each geode robot costs (\d+) ore and (\d+) obsidian./);
...
let findMaxGeodeNumber = (costs, timeRemaining) => {
    let { oreRobotOreCost, clayRobotOreCost, obsidianRobotOreCost, obsidianRobotClayCost, geodeRobotOreCost, geodeRobotObsidianCost, } = costs;
    ...
    while (stack.length) {
        ...
        let maxOreCost = Math.max(oreRobotOreCost, clayRobotOreCost, obsidianRobotOreCost, geodeRobotOreCost);
        let newBots = Object.assign({}, bots);
        let newResources = Object.assign({}, resources);
        newBots.oreRobot = Math.min(newBots.oreRobot, maxOreCost);
        newResources.ore = Math.min(newResources.ore, timeRemaining * maxOreCost - newBots.oreRobot * (timeRemaining - 1));
        newBots.clayRobot = Math.min(newBots.clayRobot, obsidianRobotClayCost);
        newResources.clay = Math.min(newResources.clay, timeRemaining * obsidianRobotClayCost -
            newBots.clayRobot * (timeRemaining - 1));
        newBots.geodeRobot = Math.min(newBots.geodeRobot, geodeRobotObsidianCost);
        newResources.obsidian = Math.min(newResources.obsidian, timeRemaining * geodeRobotObsidianCost -
            newBots.geodeRobot * (timeRemaining - 1));
        // check cache
        const cacheKey = `${JSON.stringify(newBots)}-${JSON.stringify(newResources)}-${timeRemaining}`;
        ...
        stack.push({
            bots: Object.assign({}, newBots),
            resources: Object.assign({}, updatedResources),
        ...
        if (updatedResources.ore >= geodeRobotOreCost &&
            updatedResources.obsidian >= geodeRobotObsidianCost) {
            stack.push({
                bots: Object.assign(Object.assign({}, newBots), { geodeRobot: newBots.geodeRobot + 1 }),
                resources: Object.assign(Object.assign({}, updatedResources), { ore: updatedResources.ore - geodeRobotOreCost, obsidian: updatedResources.obsidian - geodeRobotObsidianCost }),
                timeRemaining: newTimeRemaining,
            });
        }
        else if (
        ...
        updatedResources.ore >= obsidianRobotOreCost &&
            ...
            stack.push({
                bots: Object.assign(Object.assign({}, newBots), { obsidianRobot: newBots.obsidianRobot + 1 }),
                resources: Object.assign(Object.assign({}, updatedResources), { ore: updatedResources.ore - obsidianRobotOreCost, clay: updatedResources.clay - obsidianRobotClayCost }),
                timeRemaining: newTimeRemaining,
            });
        }
        else {
            ...
            if (updatedResources.ore >= oreRobotOreCost) {
                stack.push({
                    bots: Object.assign(Object.assign({}, newBots), { oreRobot: newBots.oreRobot + 1 }),
                    resources: Object.assign(Object.assign({}, updatedResources), { ore: updatedResources.ore - oreRobotOreCost }),
                    timeRemaining: newTimeRemaining,
                });
            }
            // clay robot
            if (updatedResources.ore >= clayRobotOreCost) {
                stack.push({
                    bots: Object.assign(Object.assign({}, newBots), { clayRobot: newBots.clayRobot + 1 }),
                    resources: Object.assign(Object.assign({}, updatedResources), { ore: updatedResources.ore - clayRobotOreCost }),
                    timeRemaining: newTimeRemaining,
                });
            }
        }
