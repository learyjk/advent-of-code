...
const parseBlueprints = (data) => data
    .toLowerCase()
    .split("\n")
    .map((line) => line
    .split("each ")
    ...
    const [robot, requirements] = text
        .split(".")[0]
        .split(" robot costs ");
    bots[robot] = requirements
        .split(" and ")
        .reduce((requirements, text) => {
        const [count, resource] = text.split(" ");
        requirements[resource] = parseInt(count);
        return requirements;
    }, {});
    return bots;
}, {}));
function bfs(oreBotReq, clayBotReq, obsidianBotReq, geodeBotReq, minutesLeft) {
...
function solveOne(data) {
    return parseBlueprints(data).reduce((sum, blueprint, i) => sum +
        bfs(blueprint.ore, blueprint.clay, blueprint.obsidian, blueprint.geode, 24) *
            (i + 1), 0);
}
(() => {
...
function solveTwo(data) {
    return parseBlueprints(data)
        .slice(0, 3)
        .reduce((product, blueprint) => product *
        bfs(blueprint.ore, blueprint.clay, blueprint.obsidian, blueprint.geode, 32), 1);
}
(() => {
    const data = fs.readFileSync(__dirname + "/inputExample.txt").toString();
