...
const file = (0, fs_1.readFileSync)("day-8/input.txt", "utf8");
const forest = [];
...
const checkLeft = (row, col, treeHeight) => {
...
const checkRight = (row, col, treeHeight) => {
    col++;
    while (col <= numCols - 1) {
        if (forest[row][col] >= treeHeight) {
            return false;
...
const checkDown = (row, col, treeHeight) => {
    row++;
    while (row <= numRows - 1) {
        if (forest[row][col] >= treeHeight) {
            return false;
...
const checkUp = (row, col, treeHeight) => {
    row--;
    while (row >= 0) {
        if (forest[row][col] >= treeHeight) {
            return false;
...
const checkRightScore = (row, col, treeHeight) => {
    let score = 0;
    col++;
    while (col <= numCols - 1) {
        score++;
...
const checkDownScore = (row, col, treeHeight) => {
    let score = 0;
    row++;
    while (row <= numRows - 1) {
        score++;
...
const checkUpScore = (row, col, treeHeight) => {
    let score = 0;
    row--;
    while (row >= 0) {
        score++;
