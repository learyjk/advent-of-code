...
let dirsFile = readFileSync(__dirname + "/inputDir.txt", "utf-8");
let mazeFile = readFileSync(__dirname + "/inputMaze.txt", "utf-8");

let rows = mazeFile.split("\n");

let maxX = 0;
let startPos: [number, number];
let facing: "R" | "D" | "L" | "U" = "R";
let grid: string[][] = [];
...
const updateFacing = (turnDir: "R" | "L") => {
  if (facing === "R") {
...
const getValidColumnLength = (colIndex: number): number => {
  let count = 0;
  for (let row of grid) {
...
const getNextPosRight = (pos: [number, number]): [number, number] => {
  let r = pos[0];
  let c = pos[1];
  // wrap condition
...
const getNextPosLeft = (pos: [number, number]): [number, number] => {
  let r = pos[0];
  let c = pos[1];
  // wrap condition
...
const getNextPosDown = (pos: [number, number]): [number, number] => {
  let r = pos[0];
  let c = pos[1];
  // wrap condition
...
const getNextPosUp = (pos: [number, number]): [number, number] => {
  let r = pos[0];
  let c = pos[1];
  // wrap condition
...
const getFacingValue = (facing: "R" | "L" | "U" | "D"): number => {
  if (facing === "R") return 0;
  if (facing === "D") return 1;
  if (facing === "L") return 2;
  if (facing === "U") return 3;
};
